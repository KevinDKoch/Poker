...
namespace PokerLib2Tests
{
    ...
    public class RangeTests
    {
        ...
        public void CreateRangesFromStrings()
        {
            ...
            Assert.IsTrue(range.WeightedCount() == 1);

            range = new Range("{}");
            Assert.IsTrue(range.WeightedCount() == 0);

            range = new Range("");
            Assert.IsTrue(range.WeightedCount() == 0);

            range = new Range("{AcKs,AsKc}");
            Assert.IsTrue(range.WeightedCount() == 2);

            range = new Range("{AcKs,AsKc,AcAh}");
            Assert.IsTrue(range.WeightedCount() == 3);

            range = new Range("{AK}");
            Assert.IsTrue(range.WeightedCount() == 16);

            range = new Range("{99}");
            Assert.IsTrue(range.WeightedCount() == 6);

            range = new Range("{AKo}");
            Assert.IsTrue(range.WeightedCount() == 12);

            range = new Range("{AKs(.5)}");
            Assert.IsTrue(range.WeightedCount() == 2);

            range = new Range("{A*s}");
            Assert.IsTrue(range.WeightedCount() == 12*4);

            range = new Range("{*As}");
            Assert.IsTrue(range.WeightedCount() == 12 * 4);

            range = new Range("{**s}");
            Assert.IsTrue(range.WeightedCount() == 312);

            range = new Range("{**o}");
            Assert.IsTrue(range.WeightedCount() == 936);

            range = new Range("{**}");
            Assert.IsTrue(range.WeightedCount() == 1326);

            range = new Range("{**(.5)}");
            Assert.IsTrue(range.WeightedCount() == 1326 * .5);

            range = new Range("{A5s-A2s}");
            Assert.IsTrue(range.WeightedCount() == 4 * 4);

            range = new Range("{A5s-2As}");
            Assert.IsTrue(range.WeightedCount() == 4 * 4);

            range = new Range("{22-AA}");
            Assert.IsTrue(range.WeightedCount() == 6 * 13);

            range = new Range("{77-33}");
            Assert.IsTrue(range.WeightedCount() == 6 * 5);

            range = new Range("{22-33}");
            Assert.IsTrue(range.WeightedCount() == 6 * 2);

            range = new Range("{TT-AA(.5)}");
            Assert.IsTrue(range.WeightedCount() == 5 * 6 * .5);

            range = new Range("{AKo-A3o(.75)}");
            Assert.IsTrue(range.WeightedCount() == 12 * 11 * .75 );

            range = new Range("{JT-76(.75)}");
            Assert.IsTrue(range.WeightedCount() == 16 * 5 * .75);

            range = new Range("{TT+}");
            Assert.IsTrue(range.WeightedCount() == 6 * 5);

            range = new Range("{TT-}");
            Assert.IsTrue(range.WeightedCount() == 6 * 9);

            range = new Range("{76s+}");
            Assert.IsTrue(range.WeightedCount() == 4 * 8);

            range = new Range("{76o+}");
            Assert.IsTrue(range.WeightedCount() == 12 * 8);

            range = new Range("{76+}");
            Assert.IsTrue(range.WeightedCount() == 16 * 8);
        ...
        public void TestRange(string range, double expectedCombos)
        {
            ...
            Assert.AreEqual(expectedCombos, testRange.WeightedCount(), range);
        }
        [TestMethod]
        ...
        public void Add_AddingDuplicateWeightedStartingHandOfDifferentWeight_Passes()
        {
            ...
            r.Add(new WeightedStartingHandCombo("AcKc(.255)"));
            Assert.AreEqual(4, r.Count);
            Assert.IsFalse(r.Contains(new WeightedStartingHandCombo("AcKc(.25)")));
            Assert.IsTrue(r.Contains(new WeightedStartingHandCombo("AcKc(.255)")));
        ...
        public void Equals_SameRangeCreatedWithDifferentStrings_Passes()
        {
            ...
            r1.AddRange(new Range("A5s-A2s,A*s"));
            r2.AddRange(new Range("AKs-A2s"));

            Assert.IsTrue(r1.Equals(r2), "Equality Failed with equal range strings.");

            r1.AddRange(new Range("A5s-A2s(.1),A*s(.5)"));
            r2.AddRange(new Range("AKs-A2s(.5)"));
